    ...
    void *__libc_realloc(void *oldmem, size_t bytes)
{
    ...
    if (bytes <= usable)
    {
        ...
        if ((unsigned long)difference < 2 * sizeof(INTERNAL_SIZE_T) || (chunk_is_mmapped(oldp) && difference <= GLRO(dl_pagesize)))
            return oldmem;
    }
             ...
             INTERNAL_SIZE_T nb)
{
    ...
    /* oldmem size */
    if (__builtin_expect(chunksize_nomask(oldp) <= CHUNK_HDR_SZ, 0) || __builtin_expect(oldsize >= av->system_mem, 0) || __builtin_expect(oldsize != chunksize(oldp), 0))
