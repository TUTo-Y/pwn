...
class protobuf_c_reverse:
    def __init__(self, elf_name, out_dir = '.', default_file_name = 'default'):
        ...
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
            if not os.path.exists(out_dir):
                log.error(f"目录 '{out_dir}' 创建失败，请检查权限。")
                return
...
def main():
    parser = argparse.ArgumentParser(description='逆向 Protobuf-C 生成 .proto 文件')
    # 位置参数，全部可选
    parser.add_argument('file_pos', nargs='?', help='文件名（位置参数，可选）')
    parser.add_argument('dir_pos', nargs='?', help='目录（位置参数，可选）')
    parser.add_argument('name_pos', nargs='?', help='默认名（位置参数，可选）')
    # 选项参数
    parser.add_argument('-f', '--file', help='文件名（选项参数）')
    parser.add_argument('-d', '--dir', help='目录（选项参数）')
    parser.add_argument('-n', '--name', help='默认名（选项参数）')

    args = parser.parse_args()

    # 优先选项参数，如果没有则用位置参数
    file = args.file if args.file else args.file_pos
    dir = args.dir if args.dir else args.dir_pos
    name = args.name if args.name else args.name_pos

    if not file:
        parser.error("必须指定文件名（通过位置或选项参数）")
    if not dir:
        dir = '.'
    if not name:
        name = 'default'
        
    protobuf_c_reverse(file, dir, name)
